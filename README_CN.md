...
### 环境准备
-   **ESP-IDF**：推荐使用 `v6.0.1` 版本（兼容 `v5.3` 及以上版本）。
-   **Git**：用于克隆仓库及管理子模块。
...
### 基础用法
1.  **选择开发板**：运行 `idf.py menuconfig`，进入 `WT BSP` -> `Board selection`。
2.  **初始化 BSP**：
    ```c
    #include "wt_bsp.h"
...
-   `components/wt_bsp`：BSP 核心组件。
    -   `boards`：硬件特定配置（GPIO、时序等）。
    -   `include`：公共 API 头文件。
    -   `src`：通用 BSP 接口实现。
    -   `third_party`：内置驱动库如 `led_strip` 和 `lwbtn`。
...
3.  **统一访问**：应用层调用 `wt_bsp_init()`，它会根据 Kconfig 配置自动选择正确的板级实现，随后应用即可通过通用的 `wt_bsp_get_*()` 函数获取资源。
