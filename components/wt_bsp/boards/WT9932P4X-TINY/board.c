...
#define BOARD_NAME "WT9932P4X-TINY"
...
#if WT_BSP_RGB_ENABLED
    ret = board_usb_device_uvc_status_led_start(&s_bsp_rgb);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "USB Device UVC status LED initialization failed: %s", esp_err_to_name(ret));
        s_board_is_init = true;
        board_deinit();
        return ret;
    }
